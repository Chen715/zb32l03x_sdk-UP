 ...
 * @{
 */
#if defined(HAL_PCA_MODULE_ENABLED)
...
static void PCA_OC0_SetConfig(PCA_TypeDef *PCAx, PCA_OC_InitTypeDef *OC_Config)
{
    ...
    MODIFY_REG(PCAx->CCAPM0, PCA_CCAPM0_ECOM | PCA_CCAPM0_MAT | PCA_CCAPM0_TOG | PCA_CCAPM0_PWM,
               OC_Config->CompareEnable | OC_Config->TogEnable | OC_Config->PwmEnable);

    /* Setting period */
    if(OC_Config->PwmEnable == PCA_CCAPM0_PWM)
        WRITE_REG(PCAx->CCAP0H, OC_Config->Period);
    else
    {
        WRITE_REG(PCAx->CCAP0L, OC_Config->Period);
        WRITE_REG(PCAx->CCAP0H, OC_Config->Period >> 8);
    }
    return;
}
...
static void PCA_OC1_SetConfig(PCA_TypeDef *PCAx, PCA_OC_InitTypeDef *OC_Config)
{
    ...
    /* Enable output compare and toggle function */
    MODIFY_REG(PCAx->CCAPM1, PCA_CCAPM1_ECOM | PCA_CCAPM1_MAT | PCA_CCAPM1_TOG | PCA_CCAPM1_PWM,
               OC_Config->CompareEnable | OC_Config->TogEnable | OC_Config->PwmEnable);
    ...
    {
        WRITE_REG(PCAx->CCAP1L, OC_Config->Period);
        WRITE_REG(PCAx->CCAP1H, OC_Config->Period >> 8);
    }
    return;
}
...
static void PCA_OC2_SetConfig(PCA_TypeDef *PCAx, PCA_OC_InitTypeDef *OC_Config)
{
    ...
    /* Enable output compare and toggle function */
    MODIFY_REG(PCAx->CCAPM2, PCA_CCAPM2_ECOM | PCA_CCAPM2_MAT | PCA_CCAPM2_TOG | PCA_CCAPM2_PWM,
               OC_Config->CompareEnable | OC_Config->TogEnable | OC_Config->PwmEnable);
    ...
    {
        WRITE_REG(PCAx->CCAP2L, OC_Config->Period);
        WRITE_REG(PCAx->CCAP2H, OC_Config->Period >> 8);
    }
    return;
}
...
static void PCA_OC3_SetConfig(PCA_TypeDef *PCAx, PCA_OC_InitTypeDef *OC_Config)
{
    ...
    /* Enable output compare and toggle function */
    MODIFY_REG(PCAx->CCAPM3, PCA_CCAPM3_ECOM | PCA_CCAPM3_MAT | PCA_CCAPM3_TOG | PCA_CCAPM3_PWM,
               OC_Config->CompareEnable | OC_Config->TogEnable | OC_Config->PwmEnable);
    ...
    {
        WRITE_REG(PCAx->CCAP3L, OC_Config->Period);
        WRITE_REG(PCAx->CCAP3H, OC_Config->Period >> 8);
    }
    return;
}
...
static void PCA_OC4_SetConfig(PCA_TypeDef *PCAx, PCA_OC_InitTypeDef *OC_Config)
{
    ...
    /* Enable output compare and toggle function */
    MODIFY_REG(PCAx->CCAPM4, PCA_CCAPM4_ECOM | PCA_CCAPM4_MAT | PCA_CCAPM4_TOG | PCA_CCAPM4_PWM,
               OC_Config->CompareEnable | OC_Config->TogEnable | OC_Config->PwmEnable);
    ...
    {
        WRITE_REG(PCAx->CCAP4L, OC_Config->Period);
        WRITE_REG(PCAx->CCAP4H, OC_Config->Period >> 8);
    }
    return;
}
...
HAL_StatusTypeDef HAL_PCA_IC_ConfigChannel(PCA_HandleTypeDef *hpca, PCA_IC_InitTypeDef *sConfig, uint32_t Channel)
{
    ...
    if (Channel == PCA_CHANNEL_0)
    {
        ...
        PCA_TI0_SetConfig(hpca->Instance, sConfig->ICPolarity);
    }
    else if (Channel == PCA_CHANNEL_1)
    {
        /* TI1 Channel1 input capture configuration */
        PCA_TI1_SetConfig(hpca->Instance, sConfig->ICPolarity);
    }
    else if (Channel == PCA_CHANNEL_2)
    {
        /* TI2 Channel2 input capture configuration */
        PCA_TI2_SetConfig(hpca->Instance, sConfig->ICPolarity);
    }
    else if (Channel == PCA_CHANNEL_3)
    {
        /* TI3 Channel3 input capture configuration */
        PCA_TI3_SetConfig(hpca->Instance, sConfig->ICPolarity);
    }
    else if (Channel == PCA_CHANNEL_4)
    {
        /* TI4 Channel4 input capture configuration */
        PCA_TI4_SetConfig(hpca->Instance, sConfig->ICPolarity);
    }
