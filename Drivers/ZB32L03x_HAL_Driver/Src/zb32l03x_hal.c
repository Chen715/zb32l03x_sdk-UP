 ...
 */
/**
 * @brief ZB32L03x HAL Driver version number V1.0.0
 */
#define __ZB32L03x_HAL_VERSION_MAIN     (0x00U)     /*!< [31:24] main version */
#define __ZB32L03x_HAL_VERSION_SUB1     (0x00U)     /*!< [23:16] sub1 version */
#define __ZB32L03x_HAL_VERSION_SUB2     (0x00U)     /*!< [15:8]  sub2 version */
#define __ZB32L03x_HAL_VERSION_RC       (0x00U)     /*!< [7:0]  release candidate */
#define __ZB32L03x_HAL_VERSION          ((__ZB32L03x_HAL_VERSION_MAIN << 24) | \
                                         (__ZB32L03x_HAL_VERSION_SUB1 << 16) | \
                                         (__ZB32L03x_HAL_VERSION_SUB2 << 8 ) | \
                                         (__ZB32L03x_HAL_VERSION_RC))
 ...
 *           "zb32l03x_hal_msp.c" to do the global low level hardware initialization
 *
...
HAL_StatusTypeDef HAL_DeInit(void)
{
    ...
    __HAL_RCC_OWIRE_FORCE_RESET();
    __HAL_RCC_OWIRE_RELEASE_RESET();
    ...
    __HAL_RCC_TIM2A_FORCE_RESET();
    __HAL_RCC_TIM2A_RELEASE_RESET();

    __HAL_RCC_TIM2B_FORCE_RESET();
    __HAL_RCC_TIM2B_RELEASE_RESET();

    __HAL_RCC_TIM2C_FORCE_RESET();
    __HAL_RCC_TIM2C_RELEASE_RESET();

    __HAL_RCC_TIM1A_FORCE_RESET();
    __HAL_RCC_TIM1A_RELEASE_RESET();

    __HAL_RCC_TIM1B_FORCE_RESET();
    __HAL_RCC_TIM1B_RELEASE_RESET();
    ...
    __HAL_RCC_SPII2S_FORCE_RESET();
    __HAL_RCC_SPII2S_RELEASE_RESET();
    ...
    __HAL_RCC_TRNG_FORCE_RESET();
    __HAL_RCC_TRNG_RELEASE_RESET();
...
{
    return __ZB32L03x_HAL_VERSION;
}

/**
 * @brief Returns the device revision identifier.
 * Note:
 *       Address 0x180000F0-0x180000FF are unique ID for ZB32L03x.
...
{
    UID[0] = (uint32_t)(READ_REG(*((uint32_t *)DEV_UID_BASE)));
    UID[1] = (uint32_t)(READ_REG(*((uint32_t *)(DEV_UID_BASE + 4U))));
    UID[2] = (uint32_t)(READ_REG(*((uint32_t *)(DEV_UID_BASE + 8U))));
    UID[3] = (uint32_t)(READ_REG(*((uint32_t *)(DEV_UID_BASE + 12U))));
}
