 ...
 * @retval HAL status
 */
HAL_StatusTypeDef HAL_SPI_Master_TransmitReceive(SPI_HandleTypeDef *hspi, uint8_t *pTxData, uint16_t TxSize,
...
{
    return (__HAL_SPI_GET_FLAG(hspi, SPI_FLAG_SPIF)) ? SET : RESET;
}
 ...
 * @}
 */
__weak void HAL_SPI_MasterCallback(SPI_HandleTypeDef *hspi)
{
    /* Prevent unused argument(s) compilation warning */
    UNUSED(hspi);
    /* NOTE : This function Should not be modified, when the callback is needed,
              the HAL_SPI_MasterCallback could be implemented in the user file
    */

}

__weak void HAL_SPI_SlaveCallback(SPI_HandleTypeDef *hspi)
{
    /* Prevent unused argument(s) compilation warning */
    UNUSED(hspi);
    /* NOTE : This function Should not be modified, when the callback is needed,
              the HAL_SPI_SlaveCallback could be implemented in the user file
    */
}
...
{
    uint32_t    CurrentMode = hspi->Init.Mode;

    if(CurrentMode == SPI_MODE_MASTER)
    {
        /* Master mode selected */
        HAL_SPI_MasterCallback(hspi);
    }
    else
    {
        /* Slave mode selected */
        HAL_SPI_SlaveCallback(hspi);
    }
}
