 ...
 * @param  GPIOx where x can be (A..D) to select the GPIO peripheral for ZB32L03x device
...
void HAL_GPIO_Init(GPIO_TypeDef  *GPIOx, GPIO_InitTypeDef *GPIO_Init)
{
    ...
    for(position = 0U; position < GPIO_NUMBER; position++)
    {
        ...
        if(iocurrent == ioposition)
        {
            ...
            GPIOx->INTEN &= ~(GPIO_INTEN_PxIEN_0 << (position * 1U));

            GPIOx->INTTYPCR &= ~(GPIO_INTTYPCR_PxITYPE_0 << (position * 1U));

            GPIOx->INTPOLCR &= ~(GPIO_INTPOLCR_PxIVAL_0 << (position * 1U));

            GPIOx->INTANY &= ~(GPIO_INTANY_PxIANY_0 << (position * 1U));
            ...
            if(position & 0x8)
                GPIOx->AFRH &= ~(GPIO_AFRH_PxAFR8 << ((uint32_t)(position & 0x07U) * 4U));
            else
                GPIOx->AFRL &= ~(GPIO_AFRL_PxAFR0 << ((uint32_t)(position & 0x07U) * 4U));
        #endif

            /* Configure the default value for inout mode */
            GPIOx->DIRCR &= ~(GPIO_DIRCR_PxDIR_0 << (position * 1U));

            /* Configure the default value for debounce */
            GPIOx->INDBEN &= ~(0x1ul << (position * 1U));

            /* Configure the default value for slew rate */
            GPIOx->SLEWCR |= GPIO_SLEWCR_PxSR_0 << (position * 1U);
            ...
            if(GPIO_Init->Mode == GPIO_MODE_AF)
            {
                ...
                GPIOx->AFR = temp;
            #elif defined(CONFIG_USE_ZB32L030) || defined(CONFIG_USE_ZB32L032)
                temp = (position & 0x8) ? GPIOx->AFRH : GPIOx->AFRL;
                temp &= ~(0xFU << ((uint32_t)(position & 0x07U) * 4U)) ;
                temp |= ((uint32_t)(GPIO_Init->Alternate) << (((uint32_t)position & 0x07U) * 4U));
                if(position & 0x8)  GPIOx->AFRH = temp;
                else                GPIOx->AFRL = temp;
            #endif
            }
            else if(GPIO_Init->Mode == GPIO_MODE_GPIO)
            {
            ...
            else if(GPIO_Init->Mode == GPIO_MODE_OUTPUT)
            {
                ...
                temp = GPIOx->DIRCR;
                temp &= ~(0x01U << ((uint32_t)(position & 0x0FU) * 1U)) ;
                temp |= ((uint32_t)(0x01U) << (((uint32_t)position & 0x0FU) * 1U));
                GPIOx->DIRCR = temp;
            }
            else
            {
            ...
            if(GPIO_Init->Debounce.Enable == GPIO_DEBOUNCE_ENABLE)
            {
                ...
                uint32_t        shift = (uint32_t)(position & 0x0FU) * 1U;
                temp = GPIOx->INDBEN;
                temp &= ~(0x01U << shift);
                temp |= ((!!GPIO_Init->Debounce.Enable) << shift);
                GPIOx->INDBEN = temp;
            }
            else if(GPIO_Init->Debounce.TwoLevelSync == GPIO_SYNC_ENABLE)
            {
                ...
                temp |= (!!GPIO_Init->Debounce.TwoLevelSync << GPIO_INDBEN_SYNC_EN_Pos);
                GPIOx->INDBEN = temp;
            }
            else
            {
            ...
            if((GPIO_Init->Mode & EXTI_MODE) == EXTI_MODE)
            {
                ...
                temp = GPIOx->INTEN;
                temp &= ~(GPIO_INTEN_PxIEN_0 << (position * 1U));
                temp |= ((GPIO_Init->Exti.Enable) << (position * 1U));
                GPIOx->INTEN = temp;

                temp = GPIOx->INTTYPCR;
                temp &= ~(GPIO_INTTYPCR_PxITYPE_0 << (position * 1U));
                temp |= ((GPIO_Init->Exti.EdgeLevelSel) << (position * 1U));
                GPIOx->INTTYPCR = temp;

                if((GPIO_Init->Exti.RiseFallSel == GPIO_EXTI_INT_LOWFALL) ||
                        (GPIO_Init->Exti.RiseFallSel == GPIO_EXTI_INT_HIGHRISE))
                {
                    temp = GPIOx->INTANY;
                    temp &= ~(GPIO_INTANY_PxIANY_0 << (position * 1U));
                    GPIOx->INTANY = temp;

                    temp = GPIOx->INTPOLCR;
                    temp &= ~(GPIO_INTPOLCR_PxIVAL_0 << (position * 1U));
